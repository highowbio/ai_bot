"""
Telegram-бот для дешифровки NETCFG / MXCFG файлов.
Версия 2.0 — полностью исправленная и оптимизированная.
"""
...
# ❗ ВАЖНО: Задай через переменную окружения, не хардкоди в коде!
#   Linux/Mac: export BOT_TOKEN="твой_токен"
#   Windows:   set BOT_TOKEN=твой_токен
BOT_TOKEN: str = ("8593158641:AAGhsyruVYDBgntjdL-CIlNkwD92Wt9hkVg")

# ── ИСПРАВЛЕН КРИТИЧЕСКИЙ БАГ: было "6903588929" "6734219400" ──────────────────
# Python конкатенировал строки → "69035889296734219400" = один огромный неверный ID
# Теперь правильно через запятую:
_raw_admins: str = os.getenv("6903588929,6734219400")
ADMIN_IDS: set[int] = {
...
# Rate limiting: максимум N файлов за период (секунды)
RATE_LIMIT_COUNT  = 5
...
# Rate limiting: uid → список timestamp'ов запросов
_rate_timestamps: dict[int, list[float]] = defaultdict(list)

# Асинхронный lock для whitelist (безопасность при конкурентных записях)
...
def _save_whitelist_sync(wl: set[int]) -> None:
    """Синхронная запись whitelist (вызывается через asyncio.to_thread)."""
...
async def save_whitelist(wl: set[int]) -> None:
    """Асинхронная запись whitelist с lock."""
    async with _whitelist_lock:
        await asyncio.to_thread(_save_whitelist_sync, wl)
...
def check_rate_limit(uid: int) -> bool:
    """
    Возвращает True если пользователь вписывается в лимит.
    Очищает старые записи автоматически.
    """
    now = time.monotonic()
    ts  = _rate_timestamps[uid]

    # Удаляем устаревшие
    _rate_timestamps[uid] = [t for t in ts if now - t < RATE_LIMIT_WINDOW]

    if len(_rate_timestamps[uid]) >= RATE_LIMIT_COUNT:
        return False

    _rate_timestamps[uid].append(now)
    return True
...
async def safe_edit(query, text: str, reply_markup=None) -> None:
    """
    Редактирует сообщение. При ошибке (нет текста / не изменилось) —
    отправляет новое сообщение.
    """
    kwargs: dict = dict(text=text, parse_mode="HTML", reply_markup=reply_markup)
    try:
...
async def send_long_text(
    ...
    # ── ИСПРАВЛЕН БАГ: BytesIO.name не является стандартным атрибутом ──────────
    # PTB принимает filename отдельным параметром в send_document
...
def decode_netcfg(data: bytes) -> tuple[bytes | None, str]:
    """
    Возвращает (расшифрованные_байты, статус).
    Статусы: 'already' | 'decoded' | 'too_short' | 'unknown'
    """
    if len(data) < 2:
...
def decode_mxcfg_bytes(data: bytes) -> tuple[bytes | None, str]:
    """
    Возвращает (расшифрованные_байты, статус).
    Статусы: 'already' | 'decoded' | 'empty' | 'error' | 'unknown'
    """
    if not data:
        return None, "empty"
    ...
    for key in MXCFG_KEYS:
        try:
            key_len   = len(key)
            decrypted = bytes(
                x ^ key[i % key_len] for i, x in enumerate(raw)
...
def render_scalar(value, depth: int = 0) -> str:
    """
    ── ИСПРАВЛЕН БАГ: ранее вложенные dict/list выводились как str({...}) ──
    Теперь рекурсивно обрабатываем вложенные структуры.
    """
    ...
    if isinstance(value, dict):
        if depth >= 2:
            return "{...}"
        items = ", ".join(
            f"{k}: {render_scalar(v, depth + 1)}" for k, v in list(value.items())[:5]
        )
    ...
    if isinstance(value, list):
        if depth >= 2:
            return "[...]"
        items = ", ".join(render_scalar(v, depth + 1) for v in value[:5])
        suffix = ", ..." if len(value) > 5 else ""
        return "[" + items + suffix + "]"
    return str(value)
...
def pretty_netcfg_view(data: bytes) -> str:
    """Рендерит NETCFG как JSON-дерево для просмотра."""
    # Снимаем 2-байтовый заголовок если есть
    payload = data[2:] if data[:2] in (b"\x01\x00", b"\x01\x01") else data

    try:
        text = payload.decode("utf-8", errors="ignore").strip()
    except Exception:
        return "<pre>❌ Не удалось декодировать</pre>"
    ...
    # Экранируем HTML и оборачиваем в блок кода
    escaped = pretty[:3500].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    return f"<b>📂 Содержимое NETCFG</b>\n\n<pre>{escaped}</pre>"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_allowed(uid):
        await update.message.reply_text(
            ...
            f"Ваш ID: <code>{uid}</code> — передайте его администратору.",
            parse_mode="HTML",
        )
        return
...
async def cmd_myid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Команда доступна всем — чтобы пользователь мог узнать свой ID."""
    uid = update.effective_user.id
    await update.message.reply_text(
...
async def cmd_adduser(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if target in ADMIN_IDS:
        await update.message.reply_text("ℹ️ Этот пользователь уже является администратором.")
        return
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── ИСПРАВЛЕН БАГ: query.answer() вызывался до проверки доступа ────────────
    # Telegram разрешает ответить на callback только ОДИН раз.
    # Если сначала answer() без параметров, потом с show_alert=True — второй игнорируется.
    # Теперь: сначала проверка, потом один правильный answer().
...
async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not check_rate_limit(uid):
        await update.message.reply_text(
            f"⏳ Слишком много запросов. Подожди {RATE_LIMIT_WINDOW} секунд.",
            reply_markup=kb_back_main(),
        )
        return
    ...
    try:
        tg_file = await asyncio.wait_for(doc.get_file(), timeout=30)
        buf     = BytesIO()
        await asyncio.wait_for(tg_file.download_to_memory(buf), timeout=60)
    ...
    try:
        await _process_file(update, context, uid, action, mode, raw, original_name)

    except Exception:
        logger.exception("Ошибка обработки файла uid=%s file=%s", uid, original_name)
        # ── ИСПРАВЛЕН БАГ: не раскрываем детали исключения пользователю ─────────
        await update.message.reply_text(
...
async def _process_file(
    ...
    """Обработка файла по выбранному action + mode."""
    ...
    if action == "decrypt" and mode == "netcfg":
        ...
        if result is None:
            await update.message.reply_text(
                ...
                "Убедись, что это корректный зашифрованный файл.",
                reply_markup=kb_back_main(),
            )
            return
    ...
    if action == "decrypt" and mode == "mxcfg":
        ...
        if status == "empty":
            ...
            return
        if status == "already":
            # ── ИСПРАВЛЕН БАГ: ранее файл не возвращался пользователю ──────────
            # Теперь возвращаем уже расшифрованный файл с пометкой
            buf = BytesIO(result)
            await update.message.reply_document(
                buf,
                filename=f"decoded_{original_name}",
                caption="ℹ️ Файл уже был расшифрован — возвращаю с исправленным флагом.",
                reply_markup=kb_back_main(),
            )
            user_mode.pop(uid, None)
            return
    ...
    # ── Просмотр NETCFG ───────────────────────────────────────────────────────
    if action == "view" and mode == "netcfg":
        result, status = await asyncio.to_thread(decode_netcfg, raw)
        ...
        if result is None:
            await update.message.reply_text(
                "❌ Не удалось прочитать NETCFG.",
                reply_markup=kb_back_main(),
            )
            return

        pretty = await asyncio.to_thread(pretty_netcfg_view, result)
    ...
    if action == "view" and mode == "mxcfg":
        ...
        try:
            parsed = json.loads(text)
            pretty = await asyncio.to_thread(pretty_mxcfg_view, parsed)
        except Exception:
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_allowed(uid):
        # Молча игнорируем — не раскрываем факт существования бота
        return
    ...
    if isinstance(update, Update) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "😔 Произошла непредвиденная ошибка. Попробуй /start.",
            )
...
def main() -> None:
    if not BOT_TOKEN:
        raise RuntimeError(
            "BOT_TOKEN не задан.\n"
            "Задай переменную окружения: export BOT_TOKEN=твой_токен"
        )
    if not ADMIN_IDS:
        logger.warning(
